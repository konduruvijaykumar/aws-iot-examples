 ...
 *
 */
public class NonBlockingPublisherV2 implements Runnable {
	...
	public NonBlockingPublisherV2(AWSIotMqttClient awsIotClient) {
	...
	public void run() {
		if (isSimulation) {
			...
			try {
				// Using scanner with throw exception. This code is not working
				...
				Scanner scanner = new Scanner(pythonProcess.getInputStream());
				for(;numberOfIterations > 0;){
					while(null != (line = scanner.nextLine())){
						...
						AWSIotMessage message = new NonBlockingPublishListener(PublishAndSubscribe.TestTopic, PublishAndSubscribe.TestTopicQos, payload);
